...
export default function PortfolioCard({ portfolio: p, prices, onView }: PortfolioCardProps) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-blue-100 p-0 flex flex-col h-full transition-all duration-300 hover:shadow-2xl">
      {/* Header */}
      <div className="flex items-center gap-4 px-6 pt-6 pb-2">
        <div className="w-12 h-12 bg-gradient-to-r from-blue-400 to-purple-400 rounded-xl flex items-center justify-center shadow">
          <span className="text-white font-extrabold text-2xl tracking-wide">{p.name.charAt(0)}</span>
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="text-xl sm:text-2xl font-extrabold text-blue-900 truncate mb-1">{p.name}</h2>
          <span className="inline-block bg-blue-50 text-blue-500 text-xs font-semibold rounded-full px-3 py-1">{p.visibility.replace('_', ' ')}</span>
        </div>
      </div>
      {/* Stats */}
      <div className="grid grid-cols-2 gap-3 px-6 py-4">
        <div className="bg-blue-100 rounded-lg p-3 flex flex-col items-start">
        ...
        <div className="bg-purple-100 rounded-lg p-3 flex flex-col items-start">
          <div className="flex items-center gap-2 mb-1">
        ...
        <div className={`rounded-lg p-3 flex flex-col items-start ${profitValue >= 0 ? 'bg-green-100' : 'bg-red-100'}`}> 
          <div className="flex items-center gap-2 mb-1">
        ...
        <div className="bg-blue-50 rounded-lg p-3 flex flex-col items-start">
      ...
      <div className="px-6 pb-6 mt-auto">
        <button
          onClick={() => onView(p.id)}
          className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white font-bold py-2 px-6 rounded-full hover:from-blue-600 hover:to-purple-700 transition-all duration-200 shadow-lg mt-2"
        >
          View Portfolio
        </button>
      </div>
